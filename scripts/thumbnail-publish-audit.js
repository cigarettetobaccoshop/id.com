...
function classify(product, result) {
  ...
  const productTokens = tokens(name);
  const matched = productTokens.filter((token) => urlText.includes(token)).length;
  if (matched >= Math.max(2, Math.ceil(productTokens.length * 0.75)) && Number(result.confidence || 0) >= 80) return 'exact';
...
function parseDimensions(buffer, contentType) {
  try {
    if (/png/i.test(contentType) && buffer.length >= 24 && buffer.toString('ascii', 1, 4) === 'PNG') return { width: buffer.readUInt32BE(16), height: buffer.readUInt32BE(20) };
    ...
    if (/webp/i.test(contentType) && buffer.length >= 30 && buffer.toString('ascii', 0, 4) === 'RIFF' && buffer.toString('ascii', 8, 12) === 'WEBP' && buffer.toString('ascii', 12, 16) === 'VP8X') return { width: 1 + buffer.readUIntLE(24, 3), height: 1 + buffer.readUIntLE(27, 3) };
  } catch {}
  return null;
}
...
async function resolveOne(product) {
  ...
  try {
    ...
    return { handle: product.handle, sku: product.variant_sku, title: product.title, classification, source: result?.source || null, confidence: result?.confidence ?? null, url: result?.url || null, cached: Boolean(result?.cached), visual, publishReady: classification !== 'fallback' && visual.ok && classification !== 'category', reviewRequired: classification === 'category' || classification === 'fallback' || !visual.ok };
  } catch (error) {
...
(async () => {
  ...
  const finalPublish = rows.length === 233 && publishReady === 233 && counts.fallback === 0 && counts.category === 0 && visualFailures === 0;
  const report = { generatedAt: new Date().toISOString(), origin, requested: limit, received: products.length, audited: rows.length, counts, visualFailures, publishReady, finalPublish, standard: { exact: 'verified product-level match', brand: 'brand-level match', category: 'category-only match; manual review', fallback: 'no acceptable external image; local fallback' }, rows };
