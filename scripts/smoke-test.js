...
if (!products.includes(".eq('published', true)")) throw new Error('Catalog published filter missing')
if (!products.includes(".eq('status', 'active')")) throw new Error('Catalog active-status filter missing')
...
if (!adminSession.includes('SUPABASE_SERVICE_ROLE_KEY')) throw new Error('Admin session server credential guard missing')
...
if (!adminOrders.includes('user.id !== ADMIN_UUID')) throw new Error('Admin order authorization missing')
