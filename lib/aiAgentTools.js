...
export async function cekStatusPesanan({ order_id_atau_email, verifikasi }) {
  ...
  const { data, error } = await supabaseServer.from('orders')
    .select('order_code,customer_name,customer_phone,address,city,province,postal_code,ekspedisi,payment_method,items,subtotal,shipping_cost,total,status,created_at,updated_at')
  ...
  return { verified: true, orders: [{
    ...
    payment_method: data.payment_method, courier: data.ekspedisi, subtotal: data.subtotal, shipping_cost: data.shipping_cost || 0, total: data.total ?? data.subtotal,
...
export async function executeAiTool(name, input) {
  const normalizedName = name === 'cariproduk' ? 'cari_produk' : name
