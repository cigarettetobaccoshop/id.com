...
// Public browser credentials are safe to expose; never place a service-role key
// in this module or any NEXT_PUBLIC_* variable.
const SUPABASE_URL = process.env.NEXT_PUBLIC_SUPABASE_URL || 'https://zgsbtexngystdmakqjyi.supabase.co'
const SUPABASE_PUBLISHABLE_KEY = process.env.NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY || 'sb_publishable_7wqbX7wUVFJZqinPyy8XLQ_SimByBEo'
...
function normalizeLegacySelect(columns) {
  ...
  const unquoted = columns.replace(/"([^"]+)"/g, '$1')
  return unquoted
