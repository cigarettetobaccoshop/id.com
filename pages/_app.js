...
import '../styles/r2-deep-navy-hero-v1.css'
import '../styles/r2-luxury-brand-system-v34.css'
import '../styles/r2-home-type-refinement-v35.css'
...
function GlobalInteractionGuard(){
  useEffect(()=>{
    ...
    const onClick=e=>{const theme=e.target.closest?.('.r2-icon-btn[aria-label="Tampilan"],.r2-icon-btn[data-r2-theme-toggle]');if(theme){e.preventDefault();applyTheme(root.dataset.r2Theme==='dark'?'light':'dark');return}const cart=e.target.closest?.('a.cart-link, a[aria-label="Keranjang"]');if(cart&&cart.getAttribute('href')!=='/checkout'){e.preventDefault();window.location.assign('/checkout');return}}
