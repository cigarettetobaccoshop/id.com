...
export default function AdminDashboard() {
  ...
  async function load(currentSession = session) {
    ...
    try {
      ...
      setOrders(body.orders || []); setStats(body.stats || null); setDatabase(body.database || null); setImageIntegrity(body.image_integrity || null)
      setSystem({ status:health.ok ? 'online' : 'degraded', database:health.database || 'unavailable', catalog:body.database?.active_products ?? health.catalog?.active_products ?? 0, checkedAt:health.checked_at })
  ...
  const operational = stats ? [
    ...
    ['Processing', Number(stats.processing)||0, 'processing'],
  ...
  return <>
    ...
    <main className="admin-page">
      ...
      <section className="overview-grid">
        ...
        <div className="panel system-panel"><div className="panel-head"><div><span className="section-kicker">SYSTEM PULSE</span><h2>Health Monitor</h2><p>Snapshot konektivitas production.</p></div><span className="pulse-ring"/></div><div className="system-list"><div><span>Database</span><b className={system.database==='connected'?'ok':''}>● {system.database === 'connected' ? 'Connected' : system.database}</b><small>Supabase production</small></div><div><span>Catalog</span><b>{database?.active_products ?? system.catalog ?? '—'} produk</b><small>{database?.products ?? '—'} total · active</small></div><div><span>Order Data</span><b>{database?.orders ?? stats?.total_orders ?? '—'} rows</b><small>production orders</small></div><div><span>Product Images</span><b>{database?.product_images ?? '—'} assets</b><small>{imageIntegrity?.verified ?? 0} verified · {imageIntegrity?.pending ?? 0} pending</small></div></div></div>
      </section>
      ...
      <section className="lower-grid"><div className="panel compact"><div className="panel-head"><div><span className="section-kicker">DATABASE INVENTORY</span><h2>Monitoring Data</h2><p>Snapshot tabel production yang benar-benar tersedia.</p></div><span className="panel-icon">◈</span></div><div className="info-grid"><div><span>Products</span><b>{database?.products ?? '—'} rows</b></div><div><span>Orders</span><b>{database?.orders ?? '—'} rows</b></div><div><span>Product Images</span><b>{database?.product_images ?? '—'} rows</b></div><div><span>Newsletter</span><b>{database?.newsletter_subscribers ?? '—'} rows</b></div></div></div><div className="panel compact"><div className="panel-head"><div><span className="section-kicker">DATA INTEGRITY</span><h2>Sinkronisasi Production</h2><p>Validasi endpoint admin dan database.</p></div><span className="panel-icon">✓</span></div><div className="info-grid"><div><span>Database</span><b>{system.database === 'connected' ? 'Connected' : system.database}</b></div><div><span>Images</span><b>{imageIntegrity?.verified ?? '—'} verified</b></div><div><span>Last sync</span><b>{lastSync ? lastSync.toLocaleString('id-ID',{dateStyle:'medium',timeStyle:'short'}) : '—'}</b></div><div><span>Mode</span><b>Private · No Cache</b></div></div></div></section>

      <section className="monitor-grid" aria-label="Pusat monitoring production">
        <div className="panel monitor-panel">
          <div className="panel-head"><div><span className="section-kicker">CATALOG CONTROL</span><h2>Katalog Production</h2><p>Data yang menjadi acuan tampilan katalog.</p></div><span className="panel-icon">▦</span></div>
          <div className="monitor-body">
            <div className="monitor-metric"><span>Produk aktif</span><strong>{database?.active_products ?? system.catalog ?? '—'}</strong><small>mengacu inventory production</small></div>
            <div className="monitor-metric"><span>Total produk</span><strong>{database?.products ?? '—'}</strong><small>rows tabel products</small></div>
          </div>
        </div>
        <div className="panel monitor-panel">
          <div className="panel-head"><div><span className="section-kicker">TRANSACTION WATCH</span><h2>Order Monitoring</h2><p>Prioritas operasional berdasarkan status.</p></div><span className="panel-icon">◷</span></div>
          <div className="monitor-body">
            <div className="monitor-metric"><span>Perlu perhatian</span><strong>{stats?.pending ?? '—'}</strong><small>pending order</small></div>
            <div className="monitor-metric"><span>Dalam pengiriman</span><strong>{stats?.shipped ?? '—'}</strong><small>shipped order</small></div>
            <div className="monitor-metric"><span>Selesai</span><strong>{stats?.completed ?? '—'}</strong><small>completed order</small></div>
          </div>
        </div>
        <div className="panel monitor-panel">
          <div className="panel-head"><div><span className="section-kicker">MEDIA INTEGRITY</span><h2>Asset Monitoring</h2><p>Status asset gambar yang tercatat sistem.</p></div><span className="panel-icon">◉</span></div>
          <div className="monitor-body">
            <div className="monitor-metric"><span>Assets</span><strong>{database?.product_images ?? imageIntegrity?.total ?? '—'}</strong><small>product image records</small></div>
            <div className="monitor-metric"><span>Verified</span><strong>{imageIntegrity?.verified ?? '—'}</strong><small>asset tervalidasi</small></div>
            <div className="monitor-metric"><span>Pending</span><strong>{imageIntegrity?.pending ?? '—'}</strong><small>perlu pemeriksaan</small></div>
          </div>
        </div>
        <div className="panel monitor-panel">
          <div className="panel-head"><div><span className="section-kicker">SYSTEM OBSERVABILITY</span><h2>Integritas Sistem</h2><p>Indikator aman untuk monitoring tanpa mengubah data.</p></div><span className="panel-icon">✓</span></div>
          <div className="monitor-body">
            <div className="watch-row"><span>Database</span><b className={system.database === 'connected' ? 'ok' : 'warn'}>{system.database === 'connected' ? 'CONNECTED' : String(system.database).toUpperCase()}</b></div>
            <div className="watch-row"><span>API admin</span><b className={error ? 'warn' : 'ok'}>{error ? 'CHECK REQUIRED' : 'RESPONSIVE'}</b></div>
            <div className="watch-row"><span>Auto sync</span><b className="ok">15 DETIK</b></div>
            <div className="watch-row"><span>Cache</span><b className="ok">NO-CACHE</b></div>
          </div>
        </div>
      </section>
