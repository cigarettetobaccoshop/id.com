  ...
  return <>
    ...
    <main className="catalog-app">
      ...
      {initialError?<section className="empty"><strong>Katalog belum tersedia.</strong><span>Periksa koneksi data dan coba kembali.</span></section>:shown.length===0?<section className="empty"><strong>Produk tidak ditemukan.</strong><span>Ubah kata kunci atau kategori.</span></section>:<section className="catalog-grid">{shown.map((p,i)=>{const id=keyOf(p);const favorite=favorites.includes(id);return <article className="product-card" key={id||i} role="button" tabIndex={0} onClick={()=>openDetail(p)} onKeyDown={e=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();openDetail(p)}}}>
        <div className="card-media"><Visual i={i} title={p.Title||p.Handle} favorite={favorite} onFavorite={()=>toggleFavorite(p)}/></div>
    ...
    {selected&&<div className="modal-backdrop" onClick={()=>setSelected(null)}><section className="quick-modal" onClick={e=>e.stopPropagation()}><button type="button" className="close" onClick={()=>setSelected(null)} aria-label="Tutup">×</button><Visual title={selected.Title||selected.Handle} favorite={favorites.includes(keyOf(selected))} onFavorite={()=>toggleFavorite(selected)}/><span className="category">{selected.Type||selected.Vendor||'WHOLESALE'}</span><h2>{selected.Title||selected.Handle}</h2><div className="price">{money(selected['Variant Price'])}</div><p>SKU: {selected['Variant SKU']||'—'} · Stok: {stock(selected['Variant Inventory Qty'])}</p><button type="button" className="modal-add" onClick={()=>{add(selected);setSelected(null)}}>ADD TO CART →</button></section></div>}
