...
export default function ProductsPage({ products, count, initialError, initialCatalog }) {
  ...
  return <>
    <Head><title>{catalogLabel} | Distributor Rokok Online R2 NUSANTARA</title><meta name="description" content={`${catalog === 'r2' ? r2Count : resmiCount} produk aktif di katalog R2 NUSANTARA, distributor rokok online dan grosir dari gudang Malang.`} /><link rel="canonical" href={`https://r2nusantara-shop.vercel.app/products${catalog === 'resmi' ? '?catalog=resmi' : ''}`} /><meta property="og:type" content="website"/><meta property="og:title" content={`${catalogLabel} | Distributor Rokok Online R2 NUSANTARA`} /><meta property="og:description" content={`Katalog ${catalogLabel} dari R2 NUSANTARA, distributor rokok online dan grosir gudang Malang.`} /><meta property="og:url" content={`https://r2nusantara-shop.vercel.app/products${catalog === 'resmi' ? '?catalog=resmi' : ''}`} /></Head>
    ...
    <main id="catalog-content" className="r2-catalog-clean">
      ...
      <div className="r2-clean-shell">
        ...
        <section className="r2-clean-intro"><div><span>DISTRIBUTOR ROKOK ONLINE · LIVE CATALOG</span><h1>Produk <em>{catalog === 'r2' ? 'R2 Nusantara' : 'Resmi'}</em></h1><p><strong>{catalog === 'r2' ? r2Count : resmiCount}</strong> produk aktif · <strong>{count}</strong> total live</p></div><i><b />LIVE</i></section>
