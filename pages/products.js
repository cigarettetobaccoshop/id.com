...
/* Source-of-truth mapping from the existing catalog dataset: 66 official SKUs. */
...
export default function ProductsPage({products,count,initialError}){
 const router=useRouter();const [q,setQ]=useState(''),[catalog,setCatalog]=useState('r2'),[cat,setCat]=useState('ALL'),[sort,setSort]=useState('default'),[visible,setVisible]=useState(PAGE_SIZE),[cart,setCart]=useState([]),[selected,setSelected]=useState(null),[toast,setToast]=useState(''),[favorites,setFavorites]=useState([])
