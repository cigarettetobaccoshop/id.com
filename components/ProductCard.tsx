...
function ActionButton({ children, label, onClick, className = '' }: { children: ReactNode; label: string; onClick?: () => void; className?: string }) {
  return <button type="button" aria-label={label} onClick={e => { e.stopPropagation(); onClick?.() }} className={`${styles.iconButton} ${className}`}>{children}</button>
}

export default function ProductCard({ product, onOpen, onAddToCart, onIncrease, onDecrease, onQuickView, onToggleWishlist, wishlistActive = false, priority = false }: ProductCardProps) {
  ...
  return (
    <article className={styles.card} role={onOpen ? 'button' : undefined} tabIndex={onOpen ? 0 : undefined} onClick={() => onOpen?.(product)} onKeyDown={e => { if (onOpen && (e.key === 'Enter' || e.key === ' ')) { e.preventDefault(); onOpen(product) } }}>
      ...
      <div className={styles.body}>
        ...
        <div className={styles.priceBlock}>{hasDiscount && <div className={styles.oldPriceRow}><span>{money(product.originalPrice)}</span>{product.discountPercentage ? <b>-{product.discountPercentage}%</b> : null}</div>}<strong>{money(finalPrice)}</strong></div>
        <div className={styles.stock}><i /> {product.stockStatus || 'Cek stok'}</div>
        <div className={styles.actions}>
          <div className={styles.quantity} aria-label={`Jumlah ${product.name}`} onClick={e => e.stopPropagation()}><button type="button" aria-label="Kurangi jumlah" disabled={!product.quantity} onClick={() => onDecrease?.(product)}>−</button><output aria-live="polite">{product.quantity || 0}</output><button type="button" aria-label="Tambah jumlah" onClick={() => onIncrease?.(product)}>+</button></div>
          <button type="button" className={styles.cartButton} onClick={e => { e.stopPropagation(); onAddToCart?.(product) }}>🛒 Keranjang</button>
